...
class DataRepository(private val dao: CourseDao) {
    ...
    fun getNearestSchedule(queryType: QueryType) : LiveData<Course?> {
        val filtered = nearestQuery(queryType)
        return dao.getNearestSchedule(filtered)
    }

    fun getAllCourse(sortType: SortType): LiveData<PagedList<Course>> {
        val sorted = sortedQuery(sortType)
        return dao.getAll(sorted).toLiveData(Config(PAGE_SIZE))
    }
    ...
    fun getTodaySchedule() : List<Course> {
        ...
        val todayScheduled = calendar.get(Calendar.DAY_OF_WEEK)
        return dao.getTodaySchedule(todayScheduled)
    }
